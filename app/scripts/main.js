...
class Internship {
    constructor(Name,
        ...
        this.gpa = GPA
        this.statement = Personal_Statement
...
/**
 * Display Internships datatabale
 */
function renderDatatable(data, sectionElement) {
    $('#internListTable').DataTable({
        data: data,
        columns: [
            {data: 'added', title: 'AddedDate'},
...
function startDatabaseQueries() {
    ...
    var internshipRef = firebase.database().ref('internships/');
    ...
    var internshipList = [];
    var fetchInternships = function (reference, sectionElement) {
        reference.once('value').then(function (data) {

            for(var key in data.val()){
                var internship = new Internship();
                var intern = data.val()[key];

                //var newPost = intern;
                //console.log("post: " + newPost);
                internship.name = intern.name || 'NA'
                internship.discipline = intern.discipline || 'NA'
                internship.institution = intern.institution || 'NA'
                internship.city = intern.city || 'NA'
                internship.state = intern.state || 'NA'
                internship.country = intern.country || 'NA'
                internship.description = intern.description || 'NA'
                internship.website = intern.website || 'NA'
                internship.duration = intern.duration || 'NA'
                internship.salary = intern.salary || 'NA'
                internship.courses = intern.courses || 'NA'
                internship.housing = intern.housing || 'NA'
                internship.transportation = intern.transportation || 'NA'
                internship.citizenship = intern.citizenship || 'NA'
                internship.minority = intern.minority || 'NA'
                internship.classification = intern.classification || 'NA'
                internship.gpa = intern.gpa || 'NA'
                internship.statement = intern.statement || 'NA'
                internship.recommendation = intern.recommendation || 'NA'
                internship.transcript = intern.transcript || 'NA'
                ...
                //var containerElement = sectionElement.getElementsByClassName('posts-container')[0];
                //containerElement.insertBefore(
                //    createPostElement(myUserId, internship.name, internship.discipline, internship.institution, internship.city, internship.state),
                //    containerElement.firstChild);
            }
            renderDatatable(internshipList, internshipsSection);
        });
    }
    ...
    fetchInternships(internshipRef, topUserPostsSection);
    ...
    listeningFirebaseRefs.push(internshipRef);
}
...
function cleanupUi() {
    ...
    //internshipsSection.getElementsByClassName('posts-container')[0].innerHTML = '';
